...
Labby is a self-hosted homelab dashboard: one Bun process serving a JSON API, a Svelte SPA, and an SSE stream. No database, no auth (it runs behind a LAN/VPN reverse proxy by design — see README security section). Config is a single JSON file on disk. All service credentials stay server-side; the browser only ever sees sanitized data.
