...
1. `src/server/sse/scheduler.ts` runs one `setInterval` per **enabled integration row** at `row.refreshSeconds ?? def.defaultRefreshSeconds`. Each tick calls that row's registry `def.fetch(row.config)` and `hub.publish('int:${id}', data)` (or `{ error }` on failure).
2. `src/server/sse/hub.ts` (`BroadcastHub`) caches the latest payload per stream key and fans it out to all subscribers.
3. `GET /api/stream` (in `app.ts`, via Hono `streamSSE`) replays the cached snapshot on connect (instant widget fill), then streams live updates as named SSE events (event name = the `int:${id}` key); 15s `: ping` keepalive.
4. `src/web/src/lib/stores.ts` opens **one** `EventSource`, parses `int:${id}` event names, and routes each to a per-integration Svelte store (`Map<id, store>`), flipping `streamConnected`/`stale` on drop (EventSource auto-reconnects).

**Integrations — not channels — are the unit.** Each widget is an integration *row* in the DB (`id`, `type`, `config`, `enabled`, `refreshSeconds`, `position`). `src/server/integrations/registry.ts` `INTEGRATIONS`, keyed by `IntegrationType` (`monitor`, `docker`, `qbittorrent`, …), is the real contract: it maps each type to `{ label, defaultRefreshSeconds, fields, fetch, actions? }`. SSE event name, hub cache key, store key, and scheduler timer all key off the row **id** (`int:${id}`) — so adding a new *instance* touches nothing (the scheduler picks it up on the next `startScheduler()`); adding a new *type* means one entry in `INTEGRATIONS` plus its integration client and web widget. (`Channel` / `ChannelPayload` in `src/server/types.ts` are legacy payload-shape aliases, **not** a routing contract — `type Channel = string`.)

**REST endpoints coexist with SSE.** `GET /api/*` endpoints are the initial-render fallback and are reused by the scheduler internally. After any `POST` action (container start/stop/restart, torrent pause/resume, adguard protection), `app.ts` calls `refreshIntegration(id)` to force an immediate refresh + broadcast so the UI updates without waiting for the next tick. Integration add/edit/delete/reorder calls `startScheduler()`, which re-reads the rows and rebuilds all timers.

**Config is DB-backed, validated, and never crashes.** `src/server/config/loader.ts` holds a module-level `ConfigState` (`{ok:true,config}` or `{ok:false,error}`) and notifies listeners via `onConfigChange`. The whole dashboard JSON is stored in the `settings` table under the `dashboard` key; `readConfigRaw` reads it, seeding from the default migration on first run (and persisting that default back into the DB). `saveThemeSettings` and `POST /api/settings` write to the DB and re-validate; call `reloadConfig()` to re-read. (The scheduler's timers key off integration rows, not this dashboard config — they rebuild via `startScheduler()` on integration CRUD, see above.) `src/server/config/schema.ts` is the Zod schema (discriminated union on widget `type`) plus helpers: `collectMonitorSites` (dedupes by `checkUrl`), `collectDownloadClients`, `hasWidgetType`, `sanitizeDashboard`. Invalid config → error state, server keeps running and widgets show an error.
