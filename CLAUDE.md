...
Labby is a self-hosted homelab dashboard: one Bun process serving a JSON API, a Svelte SPA, and an SSE stream. No auth (it runs behind a LAN/VPN reverse proxy by design — see README security section). Config and service credentials live in a small SQLite database (`config/labby.db`, `src/server/db.ts`), seeded on first run from `config/dashboard.example.json` and editable from the Manage Services page (the Database icon in the header). All service credentials stay server-side; the browser only ever sees sanitized data.

## Commands

```bash
bun install && (cd web && bun install)   # deps live in two places: root + web/
bun run dev                              # API + watch on :8080 (does NOT serve the web SPA)
cd web && bun run dev                    # Vite dev server with /api proxied to :8080
...
For real frontend dev, run **both** `bun run dev` and `cd web && bun run dev` — the Vite server proxies `/api` to the Hono server. The Hono server only serves the built SPA from `web/dist`, so `bun run dev` alone has no UI until you `bun run build`.
...
4. `web/src/lib/stores.ts` opens **one** `EventSource`, routes each named event to a per-widget Svelte store, and flips `streamConnected`/`stale` on drop (EventSource auto-reconnects).
...
**Theming is first-paint, zero-flash.** `web/index.html` carries `data-theme="__LABBY_THEME__"`. `app.ts` middleware string-replaces that token from the `labby_theme` cookie (else config `theme.default`, emitting `""` for "system" so the inline resolver script falls back to OS/localStorage). `POST /api/theme` sets/clears the cookie.

## Conventions

- **TypeScript strict everywhere.** Validate config and external responses with Zod / defensive parsing.
- **Types are duplicated by design** between `src/server/types.ts` and `web/src/lib/stores.ts` (server and web are separate build roots with separate tsconfigs — no shared import). Keep them in sync when changing a payload shape.
- **`@server/*` path alias** maps to `src/server/*` (root tsconfig). The web app uses `$lib` → `web/src/lib`.
- **Tests** are colocated (`*.test.ts`) and mock `fetch`; write the test alongside the integration client / schema change.
- **Icons:** the `icon` field is a prefixed string resolved by `web/src/components/Icon.svelte` — `di:` (vendored dashboard-icons, CDN fallback), `http(s)://` / `/path` (custom image), `sh:` (selfh.st), `lucide:` (inline SVG, also the universal fallback). `di:` slugs used in config must be added to the `SLUGS` array in `scripts/vendor-icons.ts` to be vendored at build time.

## Design is locked

`design/labby-ui.html` is the canonical visual reference and `design/DESIGN.md` is the token/behavior spec. Match them exactly — no ad-hoc colors. CSS tokens live in `web/src/app.css`. `backdrop-filter` (glass) is used only on the header and cards. Every widget implements loading / error / empty / ready states and respects reduced-motion.
