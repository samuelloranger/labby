<script lang="ts">
  ...
  // Initialized once from props; the parent remounts this component per open (keyed {#if}).
  let formName = $state(row?.name ?? '');
  let formEnabled = $state(row?.enabled ?? true);
  let formRefresh = $state<number | ''>(row ? (row.refreshSeconds ?? '') : meta.defaultRefreshSeconds);
  let formConfig = $state<Record<string, unknown>>(row ? { ...row.config } : defaultConfig(meta.fields));
...
<div class="form-panel" role="dialog" aria-labelledby="form-title">
  ...
  <div class="svc-fields">
    <div class="field">
      ...
      <input id="int-name" type="text" bind:value={formName} placeholder={`My ${meta.label}`} required />
    </div>
    ...
    <div class="field row-field">
      ...
      <div class="refresh-field">
        ...
        <input id="int-refresh" type="number" min="5" bind:value={formRefresh} placeholder={String(meta.defaultRefreshSeconds)} />
      </div>
    </div>
              ...
              <div class="site-row">
                ...
                <div class="site-grid">
                  ...
                  <label class="sub-field">
                    ...
                    <IconPicker value={site.icon ?? ''} onchange={(v) => updateSite(field.key, i, { icon: v })} />
                  </label>
            ...
            {#each links(field.key) as l, i (i)}
              <div class="site-row">
                ...
                <div class="site-grid">
                  ...
                  <label class="sub-field">
                    Icon
                    <IconPicker value={l.icon ?? ''} onchange={(v) => updateLink(field.key, i, { icon: v })} />
                  </label>
                  <label class="sub-field span2">
                    Link URL
