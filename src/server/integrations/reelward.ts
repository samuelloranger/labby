...
function token(): string | null {
  return process.env.REELWARD_API_TOKEN?.trim() || null;
}
...
export async function getReelwardSummary(): Promise<ReelwardPayload | { error: string }> {
  ...
  if (!key) return { error: 'REELWARD_API_TOKEN not configured' };
  ...
  try {
    ...
    return { error: err instanceof Error ? err.message : 'Reelward unreachable' };
  }
}
